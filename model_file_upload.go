...
// checks if the FileUpload type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &FileUpload{}

// FileUpload struct for FileUpload
type FileUpload struct {
	// Base64-encoded file content (max 8 MB decoded). Example: iVBORw0KGgoAAAANSUhEUgAA…
	Content string `json:"content"`
	// MIME type — an `image/_*` type or `application/pdf`. Example: image/png
	ContentType string `json:"contentType" validate:"regexp=^(?:image\\/[\\\\w.+-]+|application\\/pdf)$"`
	// Original file name — kept (sanitized) as the URL's last segment. Example: logo.png
	Filename string `json:"filename"`
}

type _FileUpload FileUpload
...
func (o FileUpload) ToMap() (map[string]interface{}, error) {
...
func (o *FileUpload) UnmarshalJSON(data []byte) (err error) {
	...
	varFileUpload := _FileUpload{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varFileUpload)

	if err != nil {
		return err
	}

	*o = FileUpload(varFileUpload)
