/*
Norbelys API

The **Norbelys API** is a single, predictable REST surface for cold email and outreach — people, senders, programs, and sending all live behind the five patterns below. Developer-first and AI-first: every name is either already invented (Schema.org) or obvious.  ## Authentication  Every request authenticates with an **org-scoped API key**. Create one in **Settings → API keys** and send it as a bearer token:  ```http GET https://api.norbelys.com/v1/people Authorization: Bearer ak_live_… ```  Interactive agents may instead use OAuth 2.1 (see `/auth.md` and the `/.well-known/oauth-protected-resource` metadata).  ## Conventions  - **Base URL** — `https://api.norbelys.com/v1`. - **JSON in, JSON out.** Timestamps are ISO-8601 in UTC. - **Cursor pagination.** List endpoints take `limit` + `cursor` and return   `{ data, hasMore, nextCursor }` (offset-paged tables add `page` + `total`). - **Expansions.** Detail GETs take an `expand[]` query param to inline related   data (e.g. `GET /people/{id}?expand[]=timeline`) instead of extra calls. - **Soft deletes.** Anything that has been used is archived, never hard-deleted —   `DELETE` archives the resource and returns it.  ## Errors  Failures return the same envelope on every 4xx/5xx, with the matching HTTP status:  ```json { \"error\": { \"type\": \"invalid_request\", \"code\": \"invalid_param\",             \"message\": \"…\", \"hint\": \"…\", \"doc_url\": \"…\" } } ```  `type` is a broad, machine-routable category derived from the status; `code` is the stable machine contract you branch on (never the human `message`). See the `Error` schema.  ## Idempotency  Every `POST` accepts an optional **`Idempotency-Key`** header. Reuse the same key to replay the original result for 24h instead of re-executing — so a retried create can never double-charge or duplicate a record.  ## Rate limits & versioning  Abuse control is enforced at the edge; responses advertise the policy via the `RateLimit-Policy` header, and a `429` carries `Retry-After`. The API is versioned in the URL path (`/v1`). Breaking changes ship under a new version; a retiring surface is announced with `Deprecation` + `Sunset` response headers at least 90 days ahead.

API version: 0.0.1
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package norbelys

import (
	"encoding/json"
)

// checks if the OrganizationUpdateParamsBouncePolicy type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &OrganizationUpdateParamsBouncePolicy{}

// OrganizationUpdateParamsBouncePolicy Bounce-handling policy to update.
type OrganizationUpdateParamsBouncePolicy struct {
	// Stop sequences on a soft delivery failure (full / over-quota mailbox).
	StopOnSoftFailure *bool `json:"stopOnSoftFailure,omitempty"`
	// Also suppress the address when a soft failure stops it.
	SuppressOnSoftStop *bool `json:"suppressOnSoftStop,omitempty"`
}

// NewOrganizationUpdateParamsBouncePolicy instantiates a new OrganizationUpdateParamsBouncePolicy object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewOrganizationUpdateParamsBouncePolicy() *OrganizationUpdateParamsBouncePolicy {
	this := OrganizationUpdateParamsBouncePolicy{}
	return &this
}

// NewOrganizationUpdateParamsBouncePolicyWithDefaults instantiates a new OrganizationUpdateParamsBouncePolicy object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewOrganizationUpdateParamsBouncePolicyWithDefaults() *OrganizationUpdateParamsBouncePolicy {
	this := OrganizationUpdateParamsBouncePolicy{}
	return &this
}

// GetStopOnSoftFailure returns the StopOnSoftFailure field value if set, zero value otherwise.
func (o *OrganizationUpdateParamsBouncePolicy) GetStopOnSoftFailure() bool {
	if o == nil || IsNil(o.StopOnSoftFailure) {
		var ret bool
		return ret
	}
	return *o.StopOnSoftFailure
}

// GetStopOnSoftFailureOk returns a tuple with the StopOnSoftFailure field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *OrganizationUpdateParamsBouncePolicy) GetStopOnSoftFailureOk() (*bool, bool) {
	if o == nil || IsNil(o.StopOnSoftFailure) {
		return nil, false
	}
	return o.StopOnSoftFailure, true
}

// HasStopOnSoftFailure returns a boolean if a field has been set.
func (o *OrganizationUpdateParamsBouncePolicy) HasStopOnSoftFailure() bool {
	if o != nil && !IsNil(o.StopOnSoftFailure) {
		return true
	}

	return false
}

// SetStopOnSoftFailure gets a reference to the given bool and assigns it to the StopOnSoftFailure field.
func (o *OrganizationUpdateParamsBouncePolicy) SetStopOnSoftFailure(v bool) {
	o.StopOnSoftFailure = &v
}

// GetSuppressOnSoftStop returns the SuppressOnSoftStop field value if set, zero value otherwise.
func (o *OrganizationUpdateParamsBouncePolicy) GetSuppressOnSoftStop() bool {
	if o == nil || IsNil(o.SuppressOnSoftStop) {
		var ret bool
		return ret
	}
	return *o.SuppressOnSoftStop
}

// GetSuppressOnSoftStopOk returns a tuple with the SuppressOnSoftStop field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *OrganizationUpdateParamsBouncePolicy) GetSuppressOnSoftStopOk() (*bool, bool) {
	if o == nil || IsNil(o.SuppressOnSoftStop) {
		return nil, false
	}
	return o.SuppressOnSoftStop, true
}

// HasSuppressOnSoftStop returns a boolean if a field has been set.
func (o *OrganizationUpdateParamsBouncePolicy) HasSuppressOnSoftStop() bool {
	if o != nil && !IsNil(o.SuppressOnSoftStop) {
		return true
	}

	return false
}

// SetSuppressOnSoftStop gets a reference to the given bool and assigns it to the SuppressOnSoftStop field.
func (o *OrganizationUpdateParamsBouncePolicy) SetSuppressOnSoftStop(v bool) {
	o.SuppressOnSoftStop = &v
}

func (o OrganizationUpdateParamsBouncePolicy) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o OrganizationUpdateParamsBouncePolicy) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.StopOnSoftFailure) {
		toSerialize["stopOnSoftFailure"] = o.StopOnSoftFailure
	}
	if !IsNil(o.SuppressOnSoftStop) {
		toSerialize["suppressOnSoftStop"] = o.SuppressOnSoftStop
	}
	return toSerialize, nil
}

type NullableOrganizationUpdateParamsBouncePolicy struct {
	value *OrganizationUpdateParamsBouncePolicy
	isSet bool
}

func (v NullableOrganizationUpdateParamsBouncePolicy) Get() *OrganizationUpdateParamsBouncePolicy {
	return v.value
}

func (v *NullableOrganizationUpdateParamsBouncePolicy) Set(val *OrganizationUpdateParamsBouncePolicy) {
	v.value = val
	v.isSet = true
}

func (v NullableOrganizationUpdateParamsBouncePolicy) IsSet() bool {
	return v.isSet
}

func (v *NullableOrganizationUpdateParamsBouncePolicy) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableOrganizationUpdateParamsBouncePolicy(val *OrganizationUpdateParamsBouncePolicy) *NullableOrganizationUpdateParamsBouncePolicy {
	return &NullableOrganizationUpdateParamsBouncePolicy{value: val, isSet: true}
}

func (v NullableOrganizationUpdateParamsBouncePolicy) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableOrganizationUpdateParamsBouncePolicy) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


