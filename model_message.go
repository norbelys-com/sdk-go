...
func (dst *Message) UnmarshalJSON(data []byte) error {
	var err error
	// try to unmarshal JSON data into InboundMessage
	err = json.Unmarshal(data, &dst.InboundMessage);
	if err == nil {
		jsonInboundMessage, _ := json.Marshal(dst.InboundMessage)
		if string(jsonInboundMessage) == "{}" { // empty struct
			dst.InboundMessage = nil
		} else {
			return nil // data stored in dst.InboundMessage, return on the first match
		}
	} else {
		dst.InboundMessage = nil
	}

	// try to unmarshal JSON data into OutboundMessage
	err = json.Unmarshal(data, &dst.OutboundMessage);
	if err == nil {
		jsonOutboundMessage, _ := json.Marshal(dst.OutboundMessage)
		if string(jsonOutboundMessage) == "{}" { // empty struct
			dst.OutboundMessage = nil
		} else {
			return nil // data stored in dst.OutboundMessage, return on the first match
		}
	} else {
		dst.OutboundMessage = nil
	}
...
func (src *Message) MarshalJSON() ([]byte, error) {
	if src.InboundMessage != nil {
		return json.Marshal(&src.InboundMessage)
	}

	if src.OutboundMessage != nil {
		return json.Marshal(&src.OutboundMessage)
	}
