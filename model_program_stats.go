/*
Norbelys API

The **Norbelys API** is a single, predictable REST surface for cold email and outreach — people, senders, programs, and sending all live behind the five patterns below. Developer-first and AI-first: every name is either already invented (Schema.org) or obvious.  ## Authentication  Every request authenticates with an **org-scoped API key**. Create one in **Settings → API keys** and send it as a bearer token:  ```http GET https://api.norbelys.com/v1/people Authorization: Bearer ak_live_… ```  Interactive agents may instead use OAuth 2.1 (see `/auth.md` and the `/.well-known/oauth-protected-resource` metadata).  ## Conventions  - **Base URL** — `https://api.norbelys.com/v1`. - **JSON in, JSON out.** Timestamps are ISO-8601 in UTC. - **Cursor pagination.** List endpoints take `limit` + `cursor` and return   `{ data, hasMore, nextCursor }` (offset-paged tables add `page` + `total`). - **Expansions.** Detail GETs take an `expand[]` query param to inline related   data (e.g. `GET /people/{id}?expand[]=timeline`) instead of extra calls. - **Soft deletes.** Anything that has been used is archived, never hard-deleted —   `DELETE` archives the resource and returns it.  ## Errors  Failures return the same envelope on every 4xx/5xx, with the matching HTTP status:  ```json { \"error\": { \"type\": \"invalid_request\", \"code\": \"invalid_param\",             \"message\": \"…\", \"hint\": \"…\", \"doc_url\": \"…\" } } ```  `type` is a broad, machine-routable category derived from the status; `code` is the stable machine contract you branch on (never the human `message`). See the `Error` schema.  ## Idempotency  Every `POST` accepts an optional **`Idempotency-Key`** header. Reuse the same key to replay the original result for 24h instead of re-executing — so a retried create can never double-charge or duplicate a record.  ## Rate limits & versioning  Abuse control is enforced at the edge; responses advertise the policy via the `RateLimit-Policy` header, and a `429` carries `Retry-After`. The API is versioned in the URL path (`/v1`). Breaking changes ship under a new version; a retiring surface is announced with `Deprecation` + `Sunset` response headers at least 90 days ahead.

API version: 0.0.1
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package norbelys

import (
	"encoding/json"
	"bytes"
	"fmt"
)

// checks if the ProgramStats type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &ProgramStats{}

// ProgramStats Per-campaign state rollup: enrollment counts + next-send schedule.
type ProgramStats struct {
	// Fraction of leads in a terminal state (0-1).
	Completion float32 `json:"completion"`
	// Live (non-archived) enrollments.
	LeadCount int32 `json:"leadCount"`
	NextSendAt interface{} `json:"nextSendAt,omitempty"`
}

type _ProgramStats ProgramStats

// NewProgramStats instantiates a new ProgramStats object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewProgramStats(completion float32, leadCount int32) *ProgramStats {
	this := ProgramStats{}
	this.Completion = completion
	this.LeadCount = leadCount
	return &this
}

// NewProgramStatsWithDefaults instantiates a new ProgramStats object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewProgramStatsWithDefaults() *ProgramStats {
	this := ProgramStats{}
	return &this
}

// GetCompletion returns the Completion field value
func (o *ProgramStats) GetCompletion() float32 {
	if o == nil {
		var ret float32
		return ret
	}

	return o.Completion
}

// GetCompletionOk returns a tuple with the Completion field value
// and a boolean to check if the value has been set.
func (o *ProgramStats) GetCompletionOk() (*float32, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Completion, true
}

// SetCompletion sets field value
func (o *ProgramStats) SetCompletion(v float32) {
	o.Completion = v
}

// GetLeadCount returns the LeadCount field value
func (o *ProgramStats) GetLeadCount() int32 {
	if o == nil {
		var ret int32
		return ret
	}

	return o.LeadCount
}

// GetLeadCountOk returns a tuple with the LeadCount field value
// and a boolean to check if the value has been set.
func (o *ProgramStats) GetLeadCountOk() (*int32, bool) {
	if o == nil {
		return nil, false
	}
	return &o.LeadCount, true
}

// SetLeadCount sets field value
func (o *ProgramStats) SetLeadCount(v int32) {
	o.LeadCount = v
}

// GetNextSendAt returns the NextSendAt field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *ProgramStats) GetNextSendAt() interface{} {
	if o == nil {
		var ret interface{}
		return ret
	}
	return o.NextSendAt
}

// GetNextSendAtOk returns a tuple with the NextSendAt field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *ProgramStats) GetNextSendAtOk() (*interface{}, bool) {
	if o == nil || IsNil(o.NextSendAt) {
		return nil, false
	}
	return &o.NextSendAt, true
}

// HasNextSendAt returns a boolean if a field has been set.
func (o *ProgramStats) HasNextSendAt() bool {
	if o != nil && !IsNil(o.NextSendAt) {
		return true
	}

	return false
}

// SetNextSendAt gets a reference to the given interface{} and assigns it to the NextSendAt field.
func (o *ProgramStats) SetNextSendAt(v interface{}) {
	o.NextSendAt = v
}

func (o ProgramStats) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o ProgramStats) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["completion"] = o.Completion
	toSerialize["leadCount"] = o.LeadCount
	if o.NextSendAt != nil {
		toSerialize["nextSendAt"] = o.NextSendAt
	}
	return toSerialize, nil
}

func (o *ProgramStats) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"completion",
		"leadCount",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varProgramStats := _ProgramStats{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varProgramStats)

	if err != nil {
		return err
	}

	*o = ProgramStats(varProgramStats)

	return err
}

type NullableProgramStats struct {
	value *ProgramStats
	isSet bool
}

func (v NullableProgramStats) Get() *ProgramStats {
	return v.value
}

func (v *NullableProgramStats) Set(val *ProgramStats) {
	v.value = val
	v.isSet = true
}

func (v NullableProgramStats) IsSet() bool {
	return v.isSet
}

func (v *NullableProgramStats) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableProgramStats(val *ProgramStats) *NullableProgramStats {
	return &NullableProgramStats{value: val, isSet: true}
}

func (v NullableProgramStats) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableProgramStats) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


