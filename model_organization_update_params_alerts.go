/*
Norbelys API

The **Norbelys API** is a single, predictable REST surface for cold email and outreach — people, senders, programs, and sending all live behind the five patterns below. Developer-first and AI-first: every name is either already invented (Schema.org) or obvious.  ## Authentication  Every request authenticates with an **org-scoped API key**. Create one in **Settings → API keys** and send it as a bearer token:  ```http GET https://api.norbelys.com/v1/people Authorization: Bearer ak_live_… ```  Interactive agents may instead use OAuth 2.1 (see `/auth.md` and the `/.well-known/oauth-protected-resource` metadata).  ## Conventions  - **Base URL** — `https://api.norbelys.com/v1`. - **JSON in, JSON out.** Timestamps are ISO-8601 in UTC. - **Cursor pagination.** List endpoints take `limit` + `cursor` and return   `{ data, hasMore, nextCursor }` (offset-paged tables add `page` + `total`). - **Expansions.** Detail GETs take an `expand[]` query param to inline related   data (e.g. `GET /people/{id}?expand[]=timeline`) instead of extra calls. - **Soft deletes.** Anything that has been used is archived, never hard-deleted —   `DELETE` archives the resource and returns it.  ## Errors  Failures return the same envelope on every 4xx/5xx, with the matching HTTP status:  ```json { \"error\": { \"type\": \"invalid_request\", \"code\": \"invalid_param\",             \"message\": \"…\", \"hint\": \"…\", \"doc_url\": \"…\" } } ```  `type` is a broad, machine-routable category derived from the status; `code` is the stable machine contract you branch on (never the human `message`). See the `Error` schema.  ## Idempotency  Every `POST` accepts an optional **`Idempotency-Key`** header. Reuse the same key to replay the original result for 24h instead of re-executing — so a retried create can never double-charge or duplicate a record.  ## Rate limits & versioning  Abuse control is enforced at the edge; responses advertise the policy via the `RateLimit-Policy` header, and a `429` carries `Retry-After`. The API is versioned in the URL path (`/v1`). Breaking changes ship under a new version; a retiring surface is announced with `Deprecation` + `Sunset` response headers at least 90 days ahead.

API version: 0.0.1
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package norbelys

import (
	"encoding/json"
)

// checks if the OrganizationUpdateParamsAlerts type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &OrganizationUpdateParamsAlerts{}

// OrganizationUpdateParamsAlerts Notification preferences to update.
type OrganizationUpdateParamsAlerts struct {
	// Turn the deliverability alert email on/off.
	Deliverability *bool `json:"deliverability,omitempty"`
}

// NewOrganizationUpdateParamsAlerts instantiates a new OrganizationUpdateParamsAlerts object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewOrganizationUpdateParamsAlerts() *OrganizationUpdateParamsAlerts {
	this := OrganizationUpdateParamsAlerts{}
	return &this
}

// NewOrganizationUpdateParamsAlertsWithDefaults instantiates a new OrganizationUpdateParamsAlerts object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewOrganizationUpdateParamsAlertsWithDefaults() *OrganizationUpdateParamsAlerts {
	this := OrganizationUpdateParamsAlerts{}
	return &this
}

// GetDeliverability returns the Deliverability field value if set, zero value otherwise.
func (o *OrganizationUpdateParamsAlerts) GetDeliverability() bool {
	if o == nil || IsNil(o.Deliverability) {
		var ret bool
		return ret
	}
	return *o.Deliverability
}

// GetDeliverabilityOk returns a tuple with the Deliverability field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *OrganizationUpdateParamsAlerts) GetDeliverabilityOk() (*bool, bool) {
	if o == nil || IsNil(o.Deliverability) {
		return nil, false
	}
	return o.Deliverability, true
}

// HasDeliverability returns a boolean if a field has been set.
func (o *OrganizationUpdateParamsAlerts) HasDeliverability() bool {
	if o != nil && !IsNil(o.Deliverability) {
		return true
	}

	return false
}

// SetDeliverability gets a reference to the given bool and assigns it to the Deliverability field.
func (o *OrganizationUpdateParamsAlerts) SetDeliverability(v bool) {
	o.Deliverability = &v
}

func (o OrganizationUpdateParamsAlerts) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o OrganizationUpdateParamsAlerts) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Deliverability) {
		toSerialize["deliverability"] = o.Deliverability
	}
	return toSerialize, nil
}

type NullableOrganizationUpdateParamsAlerts struct {
	value *OrganizationUpdateParamsAlerts
	isSet bool
}

func (v NullableOrganizationUpdateParamsAlerts) Get() *OrganizationUpdateParamsAlerts {
	return v.value
}

func (v *NullableOrganizationUpdateParamsAlerts) Set(val *OrganizationUpdateParamsAlerts) {
	v.value = val
	v.isSet = true
}

func (v NullableOrganizationUpdateParamsAlerts) IsSet() bool {
	return v.isSet
}

func (v *NullableOrganizationUpdateParamsAlerts) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableOrganizationUpdateParamsAlerts(val *OrganizationUpdateParamsAlerts) *NullableOrganizationUpdateParamsAlerts {
	return &NullableOrganizationUpdateParamsAlerts{value: val, isSet: true}
}

func (v NullableOrganizationUpdateParamsAlerts) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableOrganizationUpdateParamsAlerts) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


