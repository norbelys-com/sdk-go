/*
Norbelys API

The **Norbelys API** is a single, predictable REST surface for cold email and outreach — people, senders, programs, and sending all live behind the five patterns below. Developer-first and AI-first: every name is either already invented (Schema.org) or obvious.  ## Authentication  Every request authenticates with an **org-scoped API key**. Create one in **Settings → API keys** and send it as a bearer token:  ```http GET https://api.norbelys.com/v1/people Authorization: Bearer ak_live_… ```  Interactive agents may instead use OAuth 2.1 (see `/auth.md` and the `/.well-known/oauth-protected-resource` metadata).  ## Conventions  - **Base URL** — `https://api.norbelys.com/v1`. - **JSON in, JSON out.** Timestamps are ISO-8601 in UTC. - **Cursor pagination.** List endpoints take `limit` + `cursor` and return   `{ data, hasMore, nextCursor }` (offset-paged tables add `page` + `total`). - **Expansions.** Detail GETs take an `expand[]` query param to inline related   data (e.g. `GET /people/{id}?expand[]=timeline`) instead of extra calls. - **Soft deletes.** Anything that has been used is archived, never hard-deleted —   `DELETE` archives the resource and returns it.  ## Errors  Failures return the same envelope on every 4xx/5xx, with the matching HTTP status:  ```json { \"error\": { \"type\": \"invalid_request\", \"code\": \"invalid_param\",             \"message\": \"…\", \"hint\": \"…\", \"doc_url\": \"…\" } } ```  `type` is a broad, machine-routable category derived from the status; `code` is the stable machine contract you branch on (never the human `message`). See the `Error` schema.  ## Idempotency  Every `POST` accepts an optional **`Idempotency-Key`** header. Reuse the same key to replay the original result for 24h instead of re-executing — so a retried create can never double-charge or duplicate a record.  ## Rate limits & versioning  Abuse control is enforced at the edge; responses advertise the policy via the `RateLimit-Policy` header, and a `429` carries `Retry-After`. The API is versioned in the URL path (`/v1`). Breaking changes ship under a new version; a retiring surface is announced with `Deprecation` + `Sunset` response headers at least 90 days ahead.

API version: 0.0.1
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package norbelys

import (
	"encoding/json"
	"bytes"
	"fmt"
)

// checks if the AudienceAttributeCondition type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &AudienceAttributeCondition{}

// AudienceAttributeCondition struct for AudienceAttributeCondition
type AudienceAttributeCondition struct {
	// The field to test: a reserved field key (`email`, `firstname`, `lastname`, `telephone`, `email_domain`, `status`, `source`, `score`, `last_activity_at`, `created_at`, `group`, `program`, `suppressed`) or a custom field's code.
	Field string `json:"field"`
	Op AudienceConditionOp `json:"op"`
	Value NullableAudienceAttributeConditionValue `json:"value,omitempty"`
}

type _AudienceAttributeCondition AudienceAttributeCondition

// NewAudienceAttributeCondition instantiates a new AudienceAttributeCondition object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewAudienceAttributeCondition(field string, op AudienceConditionOp) *AudienceAttributeCondition {
	this := AudienceAttributeCondition{}
	this.Field = field
	this.Op = op
	return &this
}

// NewAudienceAttributeConditionWithDefaults instantiates a new AudienceAttributeCondition object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewAudienceAttributeConditionWithDefaults() *AudienceAttributeCondition {
	this := AudienceAttributeCondition{}
	return &this
}

// GetField returns the Field field value
func (o *AudienceAttributeCondition) GetField() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Field
}

// GetFieldOk returns a tuple with the Field field value
// and a boolean to check if the value has been set.
func (o *AudienceAttributeCondition) GetFieldOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Field, true
}

// SetField sets field value
func (o *AudienceAttributeCondition) SetField(v string) {
	o.Field = v
}

// GetOp returns the Op field value
func (o *AudienceAttributeCondition) GetOp() AudienceConditionOp {
	if o == nil {
		var ret AudienceConditionOp
		return ret
	}

	return o.Op
}

// GetOpOk returns a tuple with the Op field value
// and a boolean to check if the value has been set.
func (o *AudienceAttributeCondition) GetOpOk() (*AudienceConditionOp, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Op, true
}

// SetOp sets field value
func (o *AudienceAttributeCondition) SetOp(v AudienceConditionOp) {
	o.Op = v
}

// GetValue returns the Value field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *AudienceAttributeCondition) GetValue() AudienceAttributeConditionValue {
	if o == nil || IsNil(o.Value.Get()) {
		var ret AudienceAttributeConditionValue
		return ret
	}
	return *o.Value.Get()
}

// GetValueOk returns a tuple with the Value field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *AudienceAttributeCondition) GetValueOk() (*AudienceAttributeConditionValue, bool) {
	if o == nil {
		return nil, false
	}
	return o.Value.Get(), o.Value.IsSet()
}

// HasValue returns a boolean if a field has been set.
func (o *AudienceAttributeCondition) HasValue() bool {
	if o != nil && o.Value.IsSet() {
		return true
	}

	return false
}

// SetValue gets a reference to the given NullableAudienceAttributeConditionValue and assigns it to the Value field.
func (o *AudienceAttributeCondition) SetValue(v AudienceAttributeConditionValue) {
	o.Value.Set(&v)
}
// SetValueNil sets the value for Value to be an explicit nil
func (o *AudienceAttributeCondition) SetValueNil() {
	o.Value.Set(nil)
}

// UnsetValue ensures that no value is present for Value, not even an explicit nil
func (o *AudienceAttributeCondition) UnsetValue() {
	o.Value.Unset()
}

func (o AudienceAttributeCondition) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o AudienceAttributeCondition) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["field"] = o.Field
	toSerialize["op"] = o.Op
	if o.Value.IsSet() {
		toSerialize["value"] = o.Value.Get()
	}
	return toSerialize, nil
}

func (o *AudienceAttributeCondition) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"field",
		"op",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varAudienceAttributeCondition := _AudienceAttributeCondition{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varAudienceAttributeCondition)

	if err != nil {
		return err
	}

	*o = AudienceAttributeCondition(varAudienceAttributeCondition)

	return err
}

type NullableAudienceAttributeCondition struct {
	value *AudienceAttributeCondition
	isSet bool
}

func (v NullableAudienceAttributeCondition) Get() *AudienceAttributeCondition {
	return v.value
}

func (v *NullableAudienceAttributeCondition) Set(val *AudienceAttributeCondition) {
	v.value = val
	v.isSet = true
}

func (v NullableAudienceAttributeCondition) IsSet() bool {
	return v.isSet
}

func (v *NullableAudienceAttributeCondition) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableAudienceAttributeCondition(val *AudienceAttributeCondition) *NullableAudienceAttributeCondition {
	return &NullableAudienceAttributeCondition{value: val, isSet: true}
}

func (v NullableAudienceAttributeCondition) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableAudienceAttributeCondition) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


